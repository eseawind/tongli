	...
	<c:choose>
		<c:when test="${beans!=null && fn:length(beans)>0 }">
			...
			<tr>
				...
				<td>${bean.addres}</td>
				<td><%-- <a
					href="${basePath}/h/yc102_view.ac?id=${bean.id}"
					class="btn  btn-info" target="_blank">详情</a> --%> <a
					href="${basePath}/h/yc102_edit.ac?id=${bean.id}"
					class="btn edit green">编辑</a> <a href="javascript:void(0)"   class="btn btn-danger" 
					onclick="if(confirm('确认删除吗?')){location.href='${basePath}/h/yc102_del.ac?id=${bean.id}'};"
...
<customtag:pagingext func="loadUrlPage" params="'h/yc102_','list2','course_info6','&t=2'" />
<script>
try{
	$('._struts_5').html('${PAGEROW_OBJECT_KEY.recordCount}');
}catch(e){}
</script>