...
import cn.com.softvan.bean.wechar.reply.WxReplyPicMsg;
import cn.com.softvan.bean.wechar.reply.WxReplyTextMsg;
import cn.com.softvan.bean.wechar.reply.WxReplyVideoMsg;
import cn.com.softvan.bean.wechar.reply.WxReplyVoiceMsg;
...
//							       log.info("访问端口号："+ request.getServerPort());
	...
	private void init(){
		if(!"1".equals(jedisHelper.get("tc_wechar_info_cache_flag"))){
			...
			//已存入缓存标记  数据缓存3天
			jedisHelper.set("tc_wechar_info_cache_flag","1",3*23*60*60);
		}
	}
	/**删除当前信息缓存
	 * @param msgType
	...
	public void updateAllMsgCache(){
		...
		jedisHelper.set("tc_wechar_info_cache_flag","0");
		init();
	}
	/**
	 * redis缓存工具类取得
